...
const GUIDES = [
  ...
  },
  {
    ...
    input: '在输入框用自然语言描述任务，例如「准备下周的古代文学 presentation，需要 PPT + 讲稿，周五前完成」。可选填截止日期和优先级。',
    logic: '调用 AI（DeepSeek/Claude/GPT，取决于你在「我的」中配置的模型和 Key）→ 分析任务 → 拆解为 3-8 个有序步骤 → 每个步骤标注预估耗时（默认 20 分钟）和触发时间。可手动增删改步骤。',
    output: '步骤清单，每项可勾选完成。飞书集成后可将步骤同步为飞书任务。',
    examples: [
  ...
  },
  {
    ...
    title: '我的',
    brief: '个人设置中心：API Key、飞书账号、自我画像、能力雷达。',
    input: '可选填写：① 大模型 API Key（DeepSeek/OpenAI/Anthropic 的 Key）② 飞书 Open ID ③ Obsidian vault 路径 ④ 自我画像（一段文字描述自己的性格和认知习惯）。',
    logic: 'API Key → 庶务外包 + 雷达生成使用你的 Key 调用 AI（不填用系统默认）。飞书 ID → 签到/活动/成就通知推送到飞书。自我画像 → AI 分析生成 9 维能力雷达图（逻辑力/创造力/执行力/记忆力/专注力/情绪力/社交力/自驱力/元认知）。Vault 路径 → 学习笔记导出到本地 Obsidian。',
    output: 'AI 生成的能力雷达图（可手动微调分数和描述）。飞书实时通知。笔记本地同步。',
    examples: [
      { q: '怎么获取 API Key', a: 'DeepSeek：platform.deepseek.com → API Keys。OpenAI：platform.openai.com → API Keys。Claude：console.anthropic.com → API Keys。填到「我的」中即可使用自己的额度。' },
      { q: '没有 API Key 能用吗', a: '可以。庶务外包等功能使用系统默认 Key，但可能有频率限制。建议填自己的 Key 获得更稳定体验。' },
    ],
  },
];
